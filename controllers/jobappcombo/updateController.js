...
const generateCombo = async(comboName,val="")=>{
    ...
    if(type == 'radio' || type == 'checkbox'){
        ...
        let combo = result.map((value)=>{
            return `<label><input type="${type}" name="${name}"  id="${value.option_value}" ${val==value.option_value?'checked':''}>${value.option_value}</label>`
        });
...
exports.updateUser = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.json({data:req.body, message: "All fields are Required !" });
    }
